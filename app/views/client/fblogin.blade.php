	...
	if (response.status === 'connected') {
			...
			success : function(data) {
						...
						FB.api('/me', function(response) {
										...
										$.post(getBaseURL() + "client/ajaxfbregister", obj).success(function(data) {
									
												//parsedata = JSON.parse(data);
												if(data.exits == true)
													window.location.href ="{{ URL::to('client/payment') }}	";
													
												$("#register-form #name").val(data.name);
												$("#register-form #email").val(data.email);
												$("#register-form #email_confirmation").val(data.email);
												$("#register-form #nationality").val((data.nationality.toLowerCase()));
												$("#register-form #facebookId").val(data.facebookId);
												$("#register-form #access_token").val(data.access_token);
												$("#register-form #surname").val(data.surname);
												$("#register-form #lingua_registrazione").val(data.lingua_registrazione);
												$.unblockUI;
...
<div class="fb-login-button" ><a href="#" onClick="return fblogin();"><img src="{{ asset('images/facebook_login.png') }}" ></a></div>
